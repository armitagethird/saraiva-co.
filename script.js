  ...
  const initSpline = () => {
    ...
    spline
      ...
      .then(() => {
        ...
        // ── Start idle animation system after fade-in ──
        setTimeout(() => initIdleAnimation(canvas3d), 1200);
      })
      .catch((err) => {
...
// ══════════════════════════════════════════════════════════
// Desktop: loops infinitely → stops on mouse move → restarts after 10s idle
// Mobile: plays once on load

function initIdleAnimation(canvas) {
  const isMobile = window.matchMedia('(max-width: 992px)').matches;

  if (isMobile) {
    // Mobile: infinite pulsing animation (no touch interaction)
    playIdleSweep(canvas, true, () => true);
    return;
  }

  // ── Desktop: intelligent idle/active system ──
  ...
  function startIdleAnimation() {
    ...
    playIdleSweep(canvas, true, () => isIdleAnimating, stopIdleAnimation);
  }

  function stopIdleAnimation() {
    isIdleAnimating = false;
    // Reset idle timer — restart animation after 10s
    clearTimeout(idleTimeout);
    idleTimeout = setTimeout(startIdleAnimation, IDLE_DELAY);
  }

  // Listen for real mouse movement → stop idle animation
  const heroEl = document.getElementById('hero');
  if (heroEl) {
    heroEl.addEventListener('mousemove', (e) => {
      // Only react to REAL mouse events (not our synthetic ones)
      if (e.isTrusted) {
        if (isIdleAnimating) {
          stopIdleAnimation();
        } else {
          // Reset idle timer on every real mouse move
          clearTimeout(idleTimeout);
          idleTimeout = setTimeout(startIdleAnimation, IDLE_DELAY);
        }
      }
...
function playIdleSweep(canvas, loop, shouldContinue, onStop) {
  ...
  function animate(now) {
    ...
    if (rawT < BURST_DURATION) {
      burst = 1;
    } else if (rawT < BURST_DURATION + BURST_FADE) {
    ...
    // Fast rotation that slows down, with shrinking radius
    const spinSpeed = 0.5 - (rawT / BURST_DURATION) * 0.15; // ultra-slow dreamy spin
    const spinAngle = rawT * spinSpeed * Math.PI * 2;
    const spinRadius = 0.3 * (1 - rawT / (BURST_DURATION + BURST_FADE)); // shrinking orbit
